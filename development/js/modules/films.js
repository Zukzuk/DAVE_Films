        ...
        app.data.get_collection(target.data('directory')).success(function(response) {
          ...
          for (var i = 0; i < entries.length; i++) {
            target
              ...
              .append('<li><a href="' + app.model.server_base + target.data('directory') + '/' + entries[i].filename + '" target="_blank" class="small awesome"><i class="icon-circle"></i><i class="icon-circle-blank"></i>' + entries[i].name + '</a></li>');
          };
      ...
      function parse_films() {
        ...
        for (var i = start_item; i < end_item; i++) {
          ...
          if (film) {
            ...
            app.model.film_uri = app.model.server_base + film.data.directory + '/' + film.data.filename;
