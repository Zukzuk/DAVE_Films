...
{        
		...
		{				
			...
			else
			{
				...
				else
				{
					html += ''+
						'<img src="' + app.model.poster_uri + '" alt="' + film.name + '" border="0" width="140" height="200" class="" />'+
						'<a href="" target="_self" class="collection-button"></a>';
				}
			}
			...
			if(!film.data.poster)
			{
				html += '<div class="google-search">'+
					...
					'<a href="http://www.google.nl/search?q=' + search_name + '+' + film.year + '&hl=nl&tbo=d&source=lnms&tbm=isch&sa=X" target="_blank" >GOOGLE filmposter suggestion</a>'+
				'</div>';
			}
	...
	function get_alphabet_tile(name)
	{
		...
		if(first_letter != alphabet_letter && isNaN(first_letter))
		{
			...
			html += ''+
				'<li class="film-alphabet"><h1 class="depth" title="' + alphabet_letter.toUpperCase() + '">'+
				alphabet_letter.toUpperCase() +
				'</h1></li>';
		}
