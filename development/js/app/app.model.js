...
{	
	...
	xhr_pool : [],
	window_size : [],
	device: { type:"unknown", mobile_small:300, mobile:480, tablet_small:590, tablet:960, desktop:1400, wide:1700, full:2560 },
...
///////////////	
		...
		for(var i = 0; i < explode.length; i++)
	    {
		    ...
		    {			    	
		    	...
		    	{				    		
					...
					if(this.environment == 'production') console[this.developer_lib[j]] = function(){};
					this.developer_lib = $.grep(this.developer_lib, function(value) { return value != developer; });
				}
			}
		}
	...
	setup_routing: function(deeplink)
	{
		...
		for(i = 0; i < this.filter_types.length; i++)
		{
			...
			for(j = 1; j < deeplink.pathNames.length; j++)
			{
				...
				else if(typeof active_filters[filter_type] != 'undefined')
				{
					...
					var checklist = jQuery.grep(this.filter_types, function(value) { return value != filter_type; });
		...
		this.routing = { type:type, view:view, filters:active_filters };
			...
			else
			{
				...
				for(i = 0; i < this.routing.filters[filter_type].length; i++)
				{
					...
					if(filter_value == existing_value) 
					{
						...
						this.routing.filters[filter_type] = jQuery.grep(this.routing.filters[filter_type], function(value) { return value != filter_value; });
						break;
					}
